...
An overall formatted dataframe containing $\hat{\mathbf{\beta}} \Big(\sqrt{\widehat{var}(\hat{\mathbf{\beta}})}\Big)$ and heterogeneity p-values `p_het` to test the null hypotheses $H_{0_{\beta_{p.}}}$ can be obtained as
...
Because it is often of interest to examine associations in a case-control study on the odds ratio (OR) scale rather than the original parameter estimate scale, it is also possible to obtain a matrix containing $OR=\exp(\hat{\mathbf{\beta}})$, along with 95\% confidence intervals and heterogeneity p-values `p_het` to test the null hypotheses $H_{0_{\beta_{p.}}}$ using
...
An overall formatted dataframe containing the $\hat{\mathbf{\gamma}} \Big(\sqrt{\widehat{var}(\hat{\mathbf{\gamma}})}\Big)$ and p-values to test the null hypotheses $H_{0_{\gamma_{pk}}}$ can be obtained as
