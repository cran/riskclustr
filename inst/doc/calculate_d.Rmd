...
To estimate $D$ in the context of a case-control study, we use the `d()` function.`d()` relies on the `mlogit` function from the `mlogit` package to fit the polytomous logistic regression model. We supply `d()` with the name of the subtype variable, the number of subtypes, a list of risk factors, and the dataset name. See the function [documentation](http://www.emilyzabor.com/riskclustr/reference/d.html) for details of argument specification.
